...
const Booking = () => {
  ...
  const fetchBooking = async () => {
    const docSnap = await getDocs(query(collection(db, 'booking'), where('status', '>=', 2)));
    const docSnap1 = await getDocs(query(collection(db, 'user')));
    const docSnap2 = await getDocs(query(collection(db, 'cart')));
    const docSnap3 = await getDocs(query(collection(db, 'food'), where('restaurantId', '==', restid)));
              ...
              <TableBody>
                {showbooking.map((row, key) => (
                  ...
                  >
                    ...
                    <TableCell>{row.userInfo.no}</TableCell>
                    <TableCell>{row.dateVisit}</TableCell>
                    <TableCell>{row.timeVisit}</TableCell>
                    <TableCell>{row.tableInfo.table}</TableCell>
                    <TableCell align='center'>
                      {
                       row.status === 3 ? 'Accepted' :
                          row.status === 4 ? 'Rejected' :
                            row.status === 5 ? 'Cancelled' :''}
                      {row.status == 2 && 
                        <>
                          <Button onClick={() => acceptBooking(row.bookingId)}>Accept</Button>
